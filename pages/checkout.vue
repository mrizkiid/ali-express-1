                ...
                <div id="PlaceOrder" class="bg-white rounded-lg p-4">
                    ...
                    <form @submit.prevent="pay()">
                        ...
                        <button :disabled="isProccessing" 
                            ...
                            :class="isProccessing ? 'opacity-70' : 'opacity-100'"    
                        >
                            <Icon v-if="isProccessing" name="eos-icons:loading" />
...
const pay = async () => {
    if(currentAddress.value && currentAddress.value.data == '') {
        showError('Please add shipping addres')
        return
    }
...
const createOrder = async (stripeId) => {
    ...
    console.log('user-checkout = ',userStore.checkout);

}

const showError = (async (errorMessageText) => {
    let errorMsg = await document.querySelector("#card-error")
